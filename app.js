...
${currentUser && data.userId === currentUser.uid ? `
  <button class="btn btn-approve"
    onclick="openClaimsPanel('${doc.id}')">
    View Claims
  </button>
` : ""}
...
// ================= MULTI-CLAIM REVIEW PANEL =================

window.openClaimsPanel = async function (itemId) {

  if (!auth.currentUser) return;

  const claimsList = document.getElementById("claimsList");
  claimsList.innerHTML = "Loading claims...";
  ...
  if (snap.empty) {
    claimsList.innerHTML = "<p>No claims yet</p>";
    showSection("claimsPanel");
    return;
  }
  ...
  for (const claimDoc of snap.docs) {
    ...
    // fetch claimant info
    const userSnap = await getDocs(
      query(collection(db, "users"), where("__name__", "==", claim.claimantId))
    );
  ...
  <p><strong>Image Match:</strong> <b>${claim.similarity.toFixed(1)}%</b></p>
  <p><strong>Status:</strong> ${claim.status}</p>


      ${claim.status === "pending" ? `
        <button class="btn btn-primary"
          onclick="approveSpecificClaim('${claimDoc.id}')">
          Approve This Claim
        </button>
      ` : ""}
    `;
...
// Backend Cloud Function will notify item owner via email
};
...
// ================= APPROVE CLAIM =================
window.approveSpecificClaim = async function (claimId) {
  if (!auth.currentUser) {
    alert("Please login first");
    return;
  }
  ...
  try {
    // 1️⃣ Get claim document
    const claimSnap = await getDocs(
      query(collection(db, "claims"), where("__name__", "==", claimId))
    );
    ...
    // 2️⃣ Update claim status → approved
    await setDoc(
    ...
    // 3️⃣ Reject other claims for same item
    const otherClaimsSnap = await getDocs(
      query(
        collection(db, "claims"),
        where("itemId", "==", claimData.itemId),
        where("status", "==", "pending")
      )
    );

    for (const other of otherClaimsSnap.docs) {
      if (other.id !== claimId) {
        await setDoc(
          doc(db, "claims", other.id),
          { status: "rejected" },
          { merge: true }
        );
      }
    ...
    // 4️⃣ Create chat between owner & claimant
    ...
    // 5️⃣ Reward points to owner
    await setDoc(
      doc(db, "users", claimData.ownerId),
      { points: 5 },
      { merge: true }
    );
...
window.openChat = async function(itemId) {
  ...
  showSection("chat");
  // 🌙 Show night warning immediately
checkNightWarning();

// 🔄 Update warning every minute
setInterval(checkNightWarning, 60000);
  // 🌙 Night-time safety warning
const hour = new Date().getHours();
const warning = document.getElementById("nightWarning");
